...
measureCard() {
  ...
  // Store chart container width here where layout is guaranteed
  const chartContainer = this.shadowRoot.querySelector('.chart-container');
  this.chartContainerWidth = chartContainer ? chartContainer.offsetWidth : Math.max(0, card.offsetWidth - 32);
  ...
  renderCloudCoverageIcon(condition, coverage, isNight = false) {
    ...
    if (condition === 'partlycloudy') {
      const celestial = isNight
        ? html`<path d="M29.33 26.68a10.61 10.61 0 01-10.68-10.54A10.5 10.5 0 0119 13.5a10.54 10.54 0 1011.5 13.11 11.48 11.48 0 01-1.17.07z" fill="#cfe4ef" stroke="#72b9d5" stroke-linecap="round" stroke-linejoin="round" stroke-width="0.6"></path>`
        : html`
